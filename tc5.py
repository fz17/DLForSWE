...
import subroutine_vorticity as sv
...
index = sv.MinDistanceIndex(r, N, x, y, z)
...
tot_vort = sv.computeMetricVort(uc, gh, N, f, x, y, index)
#tot_mass         = np.sum( - gh))
print("Total inner energy=",tot_inner_energy, "\tTotal mass=",tot_mass)
print('Total vorticity = {}'.format(tot_vort))
...
with graph.as_default():
    with tf.name_scope("Input"):   
        ...
        t_vort = sv.computeMetricTensorVort(U_next, H_next, N, f, x, y, index)
        #print(x.shape, y.shape, index.shape, U_next.shape, H_next.shape)

        delta_en= tf.abs(tot_en-t_energy)
        delta_ma= tf.abs(tot_ma-t_mass)
        delta_vort = tf.abs(tot_vort - t_vort)
        ...
        loss += tf.square(delta_vort * weight)
...
with tf.Session(graph=graph) as sess:
    ...
    for nt in range(1,100):
        ...
        for train_step in range(1,500):
            [optimizer_train,delta_en_train,delta_ma_train, delta_vort_train, loss_train,U_train,H_train,value_train] = sess.run([optimizer, delta_en, delta_ma, delta_vort, loss, U_next, H_next, value], feed_dict= feed_dict_train)
            #print(">>>>en_train:",train_step, "\tloss_train=",loss_train, "\tdelta_en_train=",delta_en_train, "\tdelta_ma_train=",delta_ma_train)
            #print('EveryStep {}'.format(delta_vort_train))
            #if loss_train < 2e-2:
            #if loss_train < 2e-2:
            if delta_vort_train < 1e0 and delta_en_train < 1e0 and delta_ma_train < 1e0:
                break
        ...
        t_vort = sv.computeMetricVort(U_train, H_train, N, f, x, y, index)
        print(">>step:",nt, "\ttrain:",train_step, "\tloss=",loss_train, "\tdelta_en=",delta_en_train, "\tdelta_ma=",delta_ma_train,\
        end = ' ')
        print('delta_vort = {0:.8f}'.format(delta_vort_train))
